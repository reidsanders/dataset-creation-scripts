...
def get_args(arg_input):
    ...
    """
    parser = argparse.ArgumentParser(description="boilerplate arg parser")
    parser.add_argument("--dirpath", type=str, default="input/", help="input dir")
    parser.add_argument("--out", type=str, default="out/", help="out dir")
    parser.add_argument("--split", type=str, default="sentence", help="sentence, word")
    parser.add_argument(
        "--align", type=str, default="upperleft", help="left, upperleft, center"
    )
    ...
    parser.add_argument("--mode", type=str, default="text", help="'text' or 'font'")
    ...
    with Drawing() as draw:
        with Image(width=size[0], height=size[1], background=Color("white")) as img:
            try:
                ...
                try:
                    mutable_message = word_wrap(
                        img, draw, text, size[0] - 10, size[1] - 10
                    )
                ...
                if align == "center":
                    draw.text(
                        int(img.width / 2) - font_size // 4,
                        int(img.height / 2) + font_size // 4,
                        mutable_message,
                    )
                elif align == "left":
                    draw.text(
